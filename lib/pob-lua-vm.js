  ...
  class PoBLuaVM {
    ...
    async _doInit() {
      ...
      try {
        ...
        this._ready = true;
        console.log(`[PoBLuaVM] Engine ready in ${Math.round(performance.now() - t0)}ms`);
        return true;
      } catch (err) {
    ...
    async initBuildFromXml(xmlText) {
      ...
      try {
        ...
        console.log(`[PoBLuaVM] Build loaded in ${Math.round(performance.now() - t0)}ms`);
        return this._extractOutput(baseOutput);
      } catch (err) {
    ...
    async _readManifestFile(virtualPath) {
      ...
      return this._loadFileContent(entry.assetPath);
    }
    ...
    async _mountUpstreamFiles(entries) {
      ...
      // Mount each file into the Lua virtual filesystem
      for (let i = 0; i < entries.length; i++) {
        const entry = entries[i];
        this._engine.global.set('_tmpPath', entry.virtualPath);
        this._engine.global.set('_tmpContent', contents[i]);
        this._engine.global.set('_tmpModified', i + 1);
        await this._engine.doString(`
          _G._mountedFiles[_G._tmpPath] = _G._tmpContent
          _G._mountedMeta[_G._tmpPath] = { modified = _G._tmpModified }
        `);

        // Handle compressed timeless jewel data — use hex encoding to avoid wasmoon UTF-8 corruption
        if (entry.virtualPath.startsWith('Data/TimelessJewelData/') && entry.virtualPath.endsWith('.zip')) {
          binaryAliases.push({
            virtualPath: entry.virtualPath.replace(/\.zip$/, '.bin'),
            hex: this._inflateBinaryToHex(contents[i]),
          });
        }
        const splitMatch = entry.virtualPath.match(/^(Data\/TimelessJewelData\/.*\.zip)\.part(\d+)$/);
        if (splitMatch) {
          const base = splitMatch[1];
          const part = Number(splitMatch[2]);
          const existing = splitParts.get(base) || [];
          existing[part] = contents[i];
          splitParts.set(base, existing);
        }
      }

      // Merge split parts
      for (const [basePath, parts] of splitParts) {
        binaryAliases.push({
          virtualPath: basePath.replace(/\.zip$/, '.bin'),
          hex: this._inflateBinaryToHex(parts.map(p => p || '').join('')),
        });
      }

      // Mount binary aliases (hex-encoded to avoid UTF-8 corruption)
      for (let i = 0; i < binaryAliases.length; i++) {
        this._engine.global.set('_tmpPath', binaryAliases[i].virtualPath);
        this._engine.global.set('_tmpHex', binaryAliases[i].hex);
        this._engine.global.set('_tmpModified', entries.length + i + 1);
        await this._engine.doString(`
          _G._mountedFiles[_G._tmpPath] = _hexDecode(_G._tmpHex)
          _G._mountedMeta[_G._tmpPath] = { modified = _G._tmpModified }
          _G._tmpHex = nil
        `);
      }
      ...
      const settingsModified = entries.length + binaryAliases.length + 1;
      for (const path of ['Settings.xml', 'Path of Building/Settings.xml']) {
        this._engine.global.set('_tmpPath', path);
        this._engine.global.set('_tmpContent', DEFAULT_SETTINGS_XML);
        this._engine.global.set('_tmpModified', settingsModified);
        await this._engine.doString(`
          _G._mountedFiles[_G._tmpPath] = _G._tmpContent
          _G._mountedMeta[_G._tmpPath] = { modified = _G._tmpModified }
        `);
      ...
      await this._engine.doString('_G._tmpPath = nil; _G._tmpContent = nil; _G._tmpModified = nil');
      console.log(`[PoBLuaVM] All files mounted (${entries.length} + ${binaryAliases.length} bin aliases)`);
    }
    ...
    async _applyFileOverrides() {
      ...
      for (let i = 0; i < entries.length; i++) {
        const [virtualPath, assetPath] = entries[i];
        const content = await this._loadFileContent(assetPath);
        this._engine.global.set('_tmpPath', virtualPath);
        this._engine.global.set('_tmpContent', content);
        this._engine.global.set('_tmpModified', 100000 + i);
        await this._engine.doString(`
          _G._mountedFiles[_G._tmpPath] = _G._tmpContent
          _G._mountedMeta[_G._tmpPath] = { modified = _G._tmpModified }
        `);
